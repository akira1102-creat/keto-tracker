...
export async function renderDashboard(container) {
  ...
  <div class="page">
    ...
    <div class="status-badge ${sc.cls}">
      <div>
        <div>${sc.label}</div>
        <div style="font-size:13px;font-weight:400;opacity:0.8;margin-top:2px">${sc.sub}</div>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="progress-bar">
        <div class="progress-fill progress-calorie" style="width:${caloriePct}%"></div>
      </div>
        ...
        <div style="flex:1">
          <div class="macro-bars">
            <div class="macro-bar-row">
              ...
              <div class="progress-bar">
                <div class="progress-fill progress-fat" style="width:${Math.min(100, totals.total_fat_g / fatGoal * 100)}%"></div>
              </div>
            ...
            <div class="macro-bar-row">
              ...
              <div class="progress-bar">
                <div class="progress-fill progress-protein" style="width:${Math.min(100, totals.total_protein_g / proteinGoal * 100)}%"></div>
              </div>
            ...
            <div class="macro-bar-row">
              <div class="macro-bar-header">
                ...
                <span style="color:${totals.total_carb_g > carbLimit ? 'var(--color-danger)' : 'inherit'}">${totals.total_carb_g.toFixed(1)}g / ${carbGoal}g</span>
              </div>
              <div class="progress-bar">
                <div class="progress-fill progress-carb" style="width:${Math.min(100, totals.total_carb_g / carbGoal * 100)}%;${totals.total_carb_g > carbLimit ? 'background:var(--color-danger)' : ''}"></div>
              </div>
...
function renderMealList(meals, dateStr) {
  if (!meals.length) {
    return `<div class="empty-state" style="padding:24px 0">
      ...
      <p>今日尚未記錄任何餐點<br>點擊下方「記錄」開始</p>
    </div>`;
  }
  return `<div class="meal-list">
    ${meals.map(m => `
...
function initChart(totals) {
  ...
  macroChart = new Chart(canvas, {
    ...
    data: {
      labels: ['脂肪', '蛋白質', '碳水'],
      datasets: [{
        data: total > 0 ? [fatCal, proteinCal, carbCal] : [70, 25, 5],
    ...
    options: {
      ...
      plugins: { legend: { display: false }, tooltip: {
        callbacks: { label: ctx => `${ctx.label}: ${total > 0 ? Math.round(ctx.raw / total * 100) : ctx.raw}%` }
      }},
