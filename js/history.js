...
export async function renderHistory(container) {
  ...
  if (!logs.length) {
    container.innerHTML = `<div class="page"><div class="page-header"><span style="font-size:24px">📅</span><h1>歷史記錄</h1></div><div class="empty-state"><div class="empty-icon">📭</div><p>尚無歷史記錄<br>開始記錄你的第一餐吧！</p></div></div>`;
    return;
  }
  const byMonth = {};
  ...
  for (const [monthKey, monthLogs] of Object.entries(byMonth)) {
    ...
    for (const log of monthLogs) {
      ...
      html += `<div class="history-item" data-date="${log.date}">
    ...
    ${(log.meals || []).length === 0
      ...
      : `<div class="meal-list">${(log.meals || []).map(m => `
        <div class="meal-item">
          ...
          <div style="text-align:right;flex-shrink:0"><div class="meal-kcal">${Math.round(m.calories)}</div><div style="font-size:10px;color:var(--color-text-muted)">kcal</div></div>
        </div>`).join('')}</div>`}
    <div class="divider"></div>
    <div class="form-row" style="margin-bottom:12px">
      <div style="text-align:center;background:var(--color-surface2);border-radius:8px;padding:10px"><div style="font-size:22px;font-weight:700">${Math.round(log.total_calories || 0)}</div><div style="font-size:11px;color:var(--color-text-muted)">kcal</div></div>
      <div style="text-align:center;background:var(--color-surface2);border-radius:8px;padding:10px"><div style="font-size:22px;font-weight:700;color:var(--color-fat)">${(log.total_fat_g || 0).toFixed(1)}g</div><div style="font-size:11px;color:var(--color-text-muted)">脂肪</div></div>
    </div>
    <div class="form-row" style="margin-bottom:16px">
      <div style="text-align:center;background:var(--color-surface2);border-radius:8px;padding:10px"><div style="font-size:22px;font-weight:700;color:var(--color-protein)">${(log.total_protein_g || 0).toFixed(1)}g</div><div style="font-size:11px;color:var(--color-text-muted)">蛋白質</div></div>
      <div style="text-align:center;background:var(--color-surface2);border-radius:8px;padding:10px"><div style="font-size:22px;font-weight:700;color:var(--color-carb)">${(log.total_carb_g || 0).toFixed(1)}g</div><div style="font-size:11px;color:var(--color-text-muted)">淨碳水</div></div>
    </div>
